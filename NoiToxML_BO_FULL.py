"""
Treina NoiToxML (full, 8833 descritores) e NoiToxML (Bo, 44 descritores
Boruta fixos) para os thresholds CD-HIT 0.8 e 0.9.

Diferente do pipeline original (que reroda o BorutaPy do zero e pode
selecionar um conjunto de features diferente a cada execução, por ser
estocástico), aqui usamos diretamente as 44 features já confirmadas e
documentadas no artigo, garantindo reprodutibilidade e consistência com
o manuscrito.

IMPORTANTE: ajuste o import abaixo para o nome real do arquivo onde
'training_matrix' e 'cabecalho' estão definidos (ex: ML_treino.py).
...
# AJUSTE este import para o nome real do seu arquivo com o pipeline original

cabecalho = ['sequence', 'len', 'V', 'normV', 'K', 'normK', 'A',  'normA',
             'L', 'normL', 'T', 'normT', 'C', 'normC', 'W', 'normW','I', 'normI', 'M', 'normM','mass', 'isoelectric_point', 'net_charge',
             'gravy', 'hidro/total',
...
top_features = ['sequence','normV', 'normK', 'normA', 'normL', 'normT', 'normC', 'normW', 'normI', 'normM',
                'Norm_isoelectric_point', 'normnO', 'Norm_net_charge', 'normnN',
                ...
                'SRP', 'TPI', 'FRR', 'AHS', 'ARL', 'RRP', 'RLE', 'normQT', 'LAH', 'RRS', 'LRR', 'WRC', 'KKI', 'RLR',
                ...
                'ALW', 'normEC', 'RPR', 'normDT', 'LRK', 'PTL', 'RRG', 'normYF', 'RKR', 'GGR', 'PRP',
                'HHH', 'RIK', 'normDM', 'FAV', 'normDI', 'RFS', 'RNK', 'normDD', 'KAA', 'RAA', 'VPT', 'RSK', 'KAK',
                'AAW', 'HAH', 'IRR', 'FRF', 'KKA', 'normMC', 'normQI', 'FFN', 'NKR', 'SPF', 'KRL', 'SRA', 'IPN', 'MLK',
                'FKK', 'SWR', 'RAR', 'GSP', 'HRH', 'GAY', 'PAL', 'normNW', 'normPY', 'GRQ', 'QAR', 'RFK', 'SAW', 'LLH', 'normYC',
                'normMY', 'LKP', 'HLR', 'LRH', 'LKR', 'FRK', 'WEA', 'CAR', 'PML', 'AKS', 'FRV', 'FFR', 'RVT', 'RRW',
...
def calc_properties_test(file_path):
    ...
    for i, seq in enumerate(sequence_list):
        ...
        for j, amino in enumerate(['V', 'K', 'A', 'L', 'T', 'C', 'W', 'I', 'M']): 
        ...
        start_dipep = cabecalho.index('AA')
        den = sequence.length - 1
        plus_one = 1.0 / den
        for j in range(den):
            a1 = sequence.sequence[j]
            a2 = sequence.sequence[j + 1]
            idx = (utils.aminos[a1]['id'] * 20 + utils.aminos[a2]['id']) * 2
            matrix[i][start_dipep + idx]     += 1
            matrix[i][start_dipep + idx + 1] += plus_one
...
def training_matrix(positives_path, negatives_path):
    ...
    df = df.sample(frac=1, random_state=35, ignore_index=True)

    df.to_csv("cpps_trained_matrix_full.csv", index=False)

    label_counts = df["label"].value_counts().sort_index()
    negatives = label_counts.get(0, 0)
    positives = label_counts.get(1, 0)
...
# ── As 44 features Boruta já confirmadas (fixas, documentadas no artigo) ─────
BORUTA_44_FEATURES = [
...
assert len(BORUTA_44_FEATURES) == 44, f"Esperava 44, tem {len(BORUTA_44_FEATURES)}"
...
def cross_validate_and_report(X, y, model, label, n_splits=10, random_state=42):
    ...
    data = {
        'Teste':  [f"{round(scores['test_' + m].mean(), 4)} ({round(scores['test_' + m].std(), 4)})"
                   for m in SCORING.keys()],
        'Treino': [f"{round(scores['train_' + m].mean(), 4)} ({round(scores['train_' + m].std(), 4)})"
                   for m in SCORING.keys()]
    }
...
def make_final_model(random_state=42):
    """Mesmos hiperparâmetros usados no pipeline original para o modelo final."""
...
def train_threshold(threshold_tag, positives_path, negatives_path, random_state=42):
    print(f"\n{'='*60}")
    print(f" Treinando NoiToxML — CD-HIT {threshold_tag}")
    print(f"{'='*60}")

    # ── 1) Monta a matriz completa (8833 descritores + sequence + label) ────
    df = training_matrix(positives_path, negatives_path).copy()

    if "sequence" not in df.columns or "label" not in df.columns:
        raise ValueError("DataFrame sem as colunas obrigatórias 'sequence'/'label'.")

    feature_cols_full = [c for c in df.columns if c not in ["sequence", "label"]]
    y = df["label"].astype(int).values

    # ============================================================
    # MODELO FULL — todos os descritores (sem seleção de features)
    # ============================================================
    X_full = df[feature_cols_full].apply(pd.to_numeric, errors="coerce").fillna(0.0)
    print(f"\n[FULL] Número de descritores: {X_full.shape[1]}")

    cross_validate_and_report(X_full, y, make_final_model(random_state),
                               label=f"NoiToxML (full) - CD-HIT {threshold_tag}",
                               random_state=random_state)

    full_model = make_final_model(random_state)
    full_model.fit(X_full, y)

    full_model_path = f"extratrees_full_{threshold_tag}.pkl"
    with open(full_model_path, "wb") as f:
        pickle.dump(full_model, f)
    print(f"[FULL] Modelo salvo em: {full_model_path}")

    # ============================================================
    # MODELO BORUTA (Bo) — filtra direto para as 44 features fixas
    # ============================================================
    missing = [c for c in BORUTA_44_FEATURES if c not in df.columns]
    if missing:
        raise ValueError(
            f"As seguintes features Boruta não foram encontradas na matriz "
            f"gerada por training_matrix(): {missing}. Verifique se 'cabecalho' "
            f"no seu pipeline original contém exatamente esses nomes de coluna."
        )

    X_boruta = df[BORUTA_44_FEATURES].apply(pd.to_numeric, errors="coerce").fillna(0.0)
    print(f"\n[Bo] Número de descritores (fixos): {X_boruta.shape[1]}")

    cross_validate_and_report(X_boruta, y, make_final_model(random_state),
                               label=f"NoiToxML (Bo) - CD-HIT {threshold_tag}",
                               random_state=random_state)

    boruta_model = make_final_model(random_state)
    boruta_model.fit(X_boruta, y)

    boruta_model_path = f"extratrees_boruta_{threshold_tag}.pkl"
    with open(boruta_model_path, "wb") as f:
        pickle.dump(boruta_model, f)
    print(f"[Bo] Modelo salvo em: {boruta_model_path}")

    # Salva também a matriz reduzida do Bo, para conferência/reprodutibilidade
    df_boruta_out = df[["sequence"] + BORUTA_44_FEATURES + ["label"]].copy()
    df_boruta_out.to_csv(f"cpps_trained_matrix_boruta44_{threshold_tag}.csv", index=False)

    return {
        "full_model_path": full_model_path,
        "boruta_model_path": boruta_model_path,
        "n_samples": len(df),
    }


if __name__ == "__main__":
    # AJUSTE os caminhos abaixo para os arquivos reais de cada threshold
    result_08 = train_threshold(
        threshold_tag="08",
        positives_path="Datasets/CD08/train-positives.fasta",
        negatives_path="Datasets/CD09/train-negatives.fasta",
    )

    result_09 = train_threshold(
        threshold_tag="09",
        positives_path="Datasets/CD09/train-positives.fasta",
        negatives_path="Datasets/CD09/train-negatives.fasta",
    )

    print("\n\n=== RESUMO FINAL ===")
    print("CD-HIT 08:", result_08)
    print("CD-HIT 09:", result_09)