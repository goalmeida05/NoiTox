...
cabecalho = ['seq', 'len', 'V', 'normV', 'K', 'normK', 'A',  'normA',
...
top_features = ['seq','normV', 'normK', 'normA', 'normL', 'normT', 'normC', 'normW', 'normI', 'normM',
...
top20 = ['seq', 'normC', 'normnS', 'normnH', 'normGC','normV',
         'normM', 'normEC', 'normCP', 'norm_positively_charged_hydrophobic_CKSAAGP', 'norm_hydrophobic_positively_charged_CKSAAGP',
          'normPC', 'normL', 'Norm_isoelectric_point', 'normDC', 'normK', 'normCK', 'normSC', 'Norm_net_charge', 'Norm_mass', 'normnO', 'label']
...
def generated_trained_model(positives_path, negatives_path, top_features):
    ...
    # Salva o modelo
    pickle.dump(model, open(f'08.pkl', 'wb'))
    print(f'Model was trained and saved as 08.pkl!')

##############################################################################################################################################
################################################# TEST INDEPENDENT DATASET ####################################################################
##############################################################################################################################################
# Main Function
def process_cpps(data, model_path):

    #Load model
    model_cpp = pickle.load(open(model_path, 'rb'))
    ...
    df_cpp.to_csv(f'Results.csv', index=False)
...
help_text = """
To run HecatePEP, you only need to provide the path to the input file.
...
print("""
1) Train a new model using Hecate's strategy
2) Predict toxicity 
""")
...
if step is None:
    ...
    if positives_path is None:
        ...
        if negatives_path is None:
            ...
            print("""Choose the number of descriptors:
                  1) All 8131 
                  2) TOP 520 
                  3) TOP 20 """)
            features = input("Type 1, 2 or 3 to select the number of descriptors to train your new model based on Hecate's strategy: ")
            if features == '1':
                generated_trained_model(positives_path, negatives_path, cabecalho)
            elif features == '2':
                generated_trained_model(positives_path, negatives_path, top_features)
            elif features == '3' :
                generated_trained_model(positives_path, negatives_path, top20)
            else:
                print("Exiting...")
    ...
    if data is None:
        ...
        print("""
        1) HecatePEP model CD-HIT 08
        2) HecatePEP model CD-HIT 09
        3) HecatePEP model CD-HIT 08 TOP20
        4) HecatePEP model CD-HIT 09 TOP20
        """)
        model_select = input("Type 1, 2, 3 or 4 to select the model:")
        if model_select == '1':
            model_path = "CD08/08_model.pkl"
            process_cpps(data, model_path)
        elif model_select == '2':
            model_path = "CD08/09_model.pkl"
            process_cpps(data, model_path)
        elif model_select == '3':
            model_path = "CD08/TOP20/top20_08_model.pkl"
            process_cpps(data, model_path)
        elif model_select == '4':
            model_path = "CD08/TOP20/top20_09_model.pkl"
            process_cpps(data, model_path)
        else:
            print("Exiting...")
